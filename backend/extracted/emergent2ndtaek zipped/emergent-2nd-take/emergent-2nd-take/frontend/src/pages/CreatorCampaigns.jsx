...
export default function CreatorCampaigns() {
  ...
  const totalBudget = items.filter(c => ["PENDING_PAYMENT", "ACTIVE", "COMPLETED"].includes(c.status)).reduce((s, c) => s + (c.bounty_pool || 0), 0);
...
const CreatorCampaignCard = ({ c, onClone, navigate }) => {
  ...
  const sBg = status === "ACTIVE" ? "#2CFF05" : status === "PENDING_PAYMENT" ? "#BF00FF" :
              status === "COMPLETED" ? "#000" : "#2D2D2D";
  const sFg = status === "ACTIVE" ? "#000" : "#fff";
  const target = c.target_ocv || Math.max(1, (c.bounty_pool || 0) * 10.0);
  const progress = c.progress_percentage !== undefined ? c.progress_percentage : Math.min(100, ((c.current_ocv || c.total_ocv || 0) / target) * 100);
  ...
  return (
    <div className="oc-card" data-testid={`creator-camp-${c.campaign_id}`}>
      ...
      <div className="mt-5 grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs">
        ...
        <Stat label="OCV" value={fmtOcv(c.current_ocv || c.total_ocv || 0)} />
      ...
      <div className="mt-4">
        ...
        <div className="flex items-center justify-between mt-1 text-[10px] font-mono text-[#2D2D2D]">
          <span>{fmt(c.current_ocv || c.total_ocv || 0)} / {fmt(target)} OCV</span>
        </div>
      </div>

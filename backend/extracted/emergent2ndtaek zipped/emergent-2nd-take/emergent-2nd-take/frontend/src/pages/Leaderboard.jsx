          ...
          {top3.length > 0 && (
            <div className="grid md:grid-cols-3 gap-5 mb-8">
              {top3.map((r, i) => {
                ...
                const ocv = tab === "LIFETIME" ? r.lifetime_ocv : r.monthly_ocv;
                return (
                  <div key={r.editor.user_id} className="oc-card oc-card-offset" style={{ background: palette.bg, color: palette.fg }}
                    ...
                    <div className="mt-4 space-y-1">
                      <p className="font-display font-black text-3xl">{Number(ocv || 0).toFixed(2)} OCV</p>
                    </div>
                  </div>
                );
              })}
              ...
              return (
                <li key={r.editor.user_id} className="flex items-center gap-4 p-3 rounded-lg border-2 border-black"
                  ...
                  <div className="text-right">
                    <p className="font-display font-black text-lg">{Number(ocv || 0).toFixed(2)} OCV</p>
                  </div>
                </li>
              );
            })}

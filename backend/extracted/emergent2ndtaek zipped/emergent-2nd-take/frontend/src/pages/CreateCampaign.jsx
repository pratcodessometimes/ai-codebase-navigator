...
export default function CreateCampaign() {
  ...
  const submit = async (publish) => {
    ...
    try {
      ...
      if (publish) await publishCampaign(c.campaign_id);
      await refresh();
  ...
  const editor_pool = net * (form.editor_pool_pct / 100);
  const perf_pool   = net * (form.performance_pool_pct / 100);
  const bonus_pool  = net * (form.bonus_pool_pct / 100);
  const walletBal = Number(user?.wallet_balance || 0);
  const canPublish = walletBal >= form.bounty_pool;
  ...
  return (
    <div data-testid="create-campaign">
          ...
          <div className="space-y-4">
            ...
            <div className="grid md:grid-cols-2 gap-3">
              ...
              <Field label="MAX CLIPS PER EDITOR">
                <input type="number" value={form.max_clips_per_editor} onChange={e => setF("max_clips_per_editor", Number(e.target.value))} className="oc-input" data-testid="f-maxclips" />
              </Field>
            </div>
            <div className="grid md:grid-cols-2 gap-3">
            ...
            {/* Fund split */}
            <div className="grid md:grid-cols-5 gap-3" data-testid="fund-split">
              ...
              <Pill label="EDITOR POOL" value={`₹${fmt(editor_pool)}`} bg="#2CFF05" fg="#000" />
              <Pill label="PERFORMANCE POOL" value={`₹${fmt(perf_pool)}`} bg="#BF00FF" fg="#fff" />
              <Pill label="BONUS POOL" value={`₹${fmt(bonus_pool)}`} bg="#fff" fg="#000" border />
            </div>

            <div className="p-4 border-2 border-black rounded-lg bg-[#F8F8F8]">
              <div className="flex items-center justify-between flex-wrap gap-3">
                <div>
                  <p className="font-mono text-[10px] tracking-[0.2em] text-[#2D2D2D]">YOUR WALLET</p>
                  <p className="font-display text-2xl font-black mt-0.5">₹ {fmt(walletBal)}</p>
                </div>
                {!canPublish && (
                  <a href="/app/wallet" className="oc-btn oc-btn-secondary" data-testid="add-funds-link">
                    Add funds →
                  </a>
                )}
              </div>
              {!canPublish && (
                <p className="font-mono text-[11px] text-[#BF00FF] mt-2">
                  Need ₹{fmt(form.bounty_pool - walletBal)} more to publish this campaign.
                </p>
              )}
            </div>
          </div>
      ...
      <div className="mt-6 flex gap-3 max-w-4xl">
        ...
        {step === 4 && (
          <>
            <button onClick={() => submit(false)} disabled={busy || !form.title} className="oc-btn oc-btn-ghost" data-testid="save-draft">Save as draft</button>
            <button onClick={() => submit(true)} disabled={busy || !canPublish || !form.title} className="oc-btn oc-btn-primary" data-testid="publish-now">
              {busy ? "Publishing…" : "Fund Campaign & Publish"}
            </button>
          </>
        )}
      </div>

...
import { Share2, Pencil, Plus, Trash2, Lock, ChevronDown, ChevronUp,
         Youtube, Instagram, Twitter, MessageCircle, Music2, Check, X, ExternalLink } from "lucide-react";
    ...
    <div className="min-h-screen bg-black text-white" data-testid="public-profile">
      <header className="max-w-6xl mx-auto px-6 py-6 flex items-center justify-between">
        <a href="/" className="font-display font-black text-xl tracking-tight">
          OUT<span className="text-[#2CFF05]">CLIP</span>
        </a>
  ...
  data-testid={editable ? "profile-edit" : "profile-view"}
    >   
      ...
      <section className="grid sm:grid-cols-2 gap-5 mb-12" data-testid="metrics">
        ...
        <div className={t.card}>
          <p className={`font-mono text-[10px] tracking-[0.3em] ${t.textHint}`}>OUTCLIP POINTS EARNED</p>
          <p className={`font-display text-4xl md:text-5xl font-black mt-2 ${t.textPri}`}>{fmt(data.total_points_earned)}</p>
          <p className="font-mono text-xs text-[#2CFF05] mt-2">LIFETIME</p>
        </div>
      </section>
                  ...
                  <tbody>
                    {data.campaign_history.map(h => (
                      <tr key={h.campaign_id} className={`border-t ${theme === "dark" ? "border-[#2D2D2D]" : "border-black"}`}>
                        ...
                        <td className={`py-3 text-right ${t.textPri}`}>{fmt(h.points)}</td>
  ...
  return (
    <div className={`${t.card} group relative`} data-testid={`clip-card-${clip.id}`}>
      ...
      <div className="mt-5 flex items-center justify-between">
        ...
        <span className="oc-chip" style={{ background: "#2CFF05", color: "#000" }}>+{fmt(clip.points)} POINTS</span>
      </div>
    </div>
  );
};
...
const AddClipModal = ({ theme, onClose, onDone }) => {
  ...
  const [form, setForm] = useState({
    title: "", platform: "YOUTUBE_SHORTS", views: 0, points: 0, video_url: "", thumbnail_url: "",
  });
  ...
  const submit = async () => {
    ...
    try {
      ...
      await addFeaturedClip({
        ...
        views: Number(form.views), points: Number(form.points),
      ...
      <div className={`${t.card} w-full max-w-lg max-h-[90vh] overflow-y-auto`}>
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <Field t={t} label="OUTCLIP POINTS">
              <input type="number" value={form.points} onChange={e => setForm({ ...form, points: e.target.value })} className={t.inputBase} data-testid="add-points" />
            </Field>
          </div>
